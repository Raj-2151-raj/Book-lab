      ...
      {popularBooks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {popularBooks.map((book) => (
            <div key={book.id} className="bg-white m-auto p-4 rounded shadow hover:-translate-y-1 duration-500">
              <h3 className="text-lg font-semibold">{book.title}</h3>
               <img src={book.coverImage} alt= {book.title} className='w-70 h-60 rounded-xl flex flex-col  items-center justify-center' />
